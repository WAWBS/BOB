...
void food() {  //生成食物
    int $, y;
    while (1) {
        $ = (int)(15 * rand() / (RAND_MAX + 1.0));  //随机产生一组食物坐标
        y = (int)(15 * rand() / (RAND_MAX + 1.0));
        if (map[$][y] == ' ') {  //如果是空格则在该处生成食物
            map[$][y] = 'O';
            //foodx = $;  //记录食物坐标 
            //foody = y;
            break;
...
void move() {  //蛇移动
    ...
    if (t == sum) {  //没有吃到苹果
        for (i = 0; i < sum - 1; i++) {
            ...
            map[A[i].$][A[i].y] = '$';  //把地图上蛇身坐标处的字符设置成‘$’
        }
        A[sum - 1].$ = A[sum - 1].$ + dx[A[sum - 1].dir];  //蛇头按当前方向移动一格 
        A[sum - 1].y = A[sum - 1].y + dy[A[sum - 1].dir];
        map[A[sum - 1].$][A[sum - 1].y] = '@';  //把地图上蛇头坐标处的字符设置成‘@’
    }
    else {  //吃到苹果（sum会加1）
        map[A[sum - 2].$][A[sum - 2].y] = '$';  //把地图上原蛇头坐标处的字符设置成‘$’
        A[sum - 1].$ = A[sum - 2].$ + dx[A[sum - 2].dir];  //新蛇头的坐标是原蛇头沿当前方向移动一格后的坐标 
...
void check() {  //检查是否死亡或者吃到食物
    int $, y, i, j;
    $ = A[sum - 1].$ + dx[A[sum - 1].dir];  //记录按当前方向移动一格后蛇头的坐标 
    y = A[sum - 1].y + dy[A[sum - 1].dir];
    if (map[$][y] == '*' || map[$][y] == '$') {  //如果地图上该坐标处字符为‘*’或‘$’就死亡 
        if ($ != A[0].$ || y != A[0].y) {  //蛇尾除外 
